...
export default function ProfileModal({ username, games, onClose }: ProfileModalProps) {
  const [shareLink, setShareLink] = useState<string | null>(null);
  const [copied, setCopied] = useState(false);

  const calculateStats = () => {
    const totalGames = games.length;
    const playedGames = games.filter(game => game.playTime && game.playTime !== "00:00:00").length;
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-zinc-900 w-full max-w-2xl rounded-lg shadow-xl m-4">
        <div className="p-6">
